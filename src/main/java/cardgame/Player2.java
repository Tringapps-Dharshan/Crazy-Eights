...
public class Player2 implements PlayerStrategy {
    ...
    public void receiveCard(Card drawnCard) {
        System.out.println("Sherin recieved: " + drawnCard.getRank() + " " + drawnCard.getSuit());
    ...
    public Card playCard() {
        ...
        if (changedSuit == null) {
            for (int i = 0; i < myCards.size(); i++) {
                if (myCards.get(i).getSuit().equals(topPileCard.getSuit())
                        || myCards.get(i).getRank().equals(topPileCard.getRank())) {
                    System.out.println("Shrein played: " + myCards.get(i).getRank() + " " + myCards.get(i).getSuit());
            ...
            for (int i = 0; i < myCards.size(); i++) {
                if (myCards.get(i).getSuit().equals(changedSuit)) {
                    System.out.println("Sherin played: " + myCards.get(i).getRank() + " " + myCards.get(i).getSuit());
                    outCard = myCards.get(i);
                    myCards.remove(i);
                    break;
                }
